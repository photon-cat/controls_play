...
class Controller(BaseController):
  """
  PID controller with preview feed-forward and output smoothing, all gain-scheduled
  """
  def __init__(self):
    # Gains at [0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40] m/s
    self.v_points = np.arange(0, 41, 4)
    self.p_points = np.array([0.195, 0.195, 0.195, 0.195, 0.195, 0.195, 0.195, 0.195, 0.195, 0.195, 0.1])
    self.i_points = np.array([0.100, 0.100, 0.100, 0.100, 0.100, 0.100, 0.100, 0.100, 0.100, 0.100, 0.05])
    ...
    # Output smoothing factor (alpha)
    self.u_alpha_points = np.array([0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.9])
  ...
  def update(self, target_lataccel, current_lataccel, state, future_plan):
    ...
    u_alpha = np.interp(v_ego, self.v_points, self.u_alpha_points)
    ...
    # --- output smoothing ---
    u = (1.0 - u_alpha) * self.u_prev + u_alpha * u_raw
    self.u_prev = u
